...
## Compile and running on the Java command line:
	...
	* For Windows: 
		1. Just change into the project root directory and use the command
					...
					javac -cp .\lib\* .\src\TicketViewerTests\*.java .\src\TicketViewerCode\*.java
		...
		2. from project root directory run the program using the java command
					...
					java -cp .\lib\json-simple-1.1.1.jar;.\src TicketViewerCode.Menu
					
## Running the test:
	* For Windows:  from project root directory run the tests using the java command

					java -cp .\lib\*;.\src TicketViewerTests.TestRunner
